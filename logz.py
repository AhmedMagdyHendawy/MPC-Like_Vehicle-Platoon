...
def configure_output_dir(d,i):
    ...
    G.output_file = open(osp.join(G.output_dir,"log%s.txt"%i), 'w')
...
def pickle_tf_vars(scope):  
    """
    Saves tensorflow variables
    Requires them to be initialized first, also a default session must exist
    """
    _dict = {v.name : v.eval() for v in tf.global_variables(scope)}
    with open(osp.join(G.output_dir, "vars%s.pkl"%scope), 'wb') as f:
        pickle.dump(_dict, f)
